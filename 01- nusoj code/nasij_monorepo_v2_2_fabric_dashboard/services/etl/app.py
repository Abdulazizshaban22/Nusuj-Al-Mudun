...
# ---------- Studio scores (derived from indicators + IoT) ----------
@app.get("/studio/scores")
def studio_scores(studio: str = "Mobility"):
    REQS.labels(endpoint="/studio/scores").inc()
    with db() as conn:
        rows = conn.execute("SELECT uc.id, ST_Area(uc.geom::geography)/1e6 AS area_km2, fi.efficiency, fi.resilience, fi.connectivity FROM urban_cells uc LEFT JOIN fabric_indicators fi ON fi.cell_id=uc.id ORDER BY uc.id").fetchall()
        # IoT: potential extension - spatial join recent events for environment/wellbeing
    scores = []
    for (cid, area, eff, res, con) in rows:
        eff = float(eff or 0.6); res = float(res or 0.6); con = float(con or 0.6)
        if studio.lower()=="mobility":
            s = 0.7*con + 0.3*eff
        elif studio.lower()=="green":
            s = 0.6*res + 0.2*eff + 0.2*con
        elif studio.lower()=="safety":
            s = 0.5*res + 0.3*con + 0.2*eff
        elif studio.lower()=="energy":
            s = 0.6*eff + 0.3*res + 0.1*con
        elif studio.lower()=="water":
            s = 0.5*res + 0.3*eff + 0.2*con
        elif studio.lower()=="housing":
            s = 0.5*eff + 0.3*con + 0.2*res
        elif studio.lower()=="economy":
            s = 0.5*eff + 0.3*res + 0.2*con
        elif studio.lower()=="wellbeing":
            s = 0.5*res + 0.3*eff + 0.2*con
        elif studio.lower()=="governance":
            s = 0.34*eff + 0.33*res + 0.33*con
        elif studio.lower()=="heritage":
            s = 0.5*res + 0.25*eff + 0.25*con
        else:
            s = (eff+res+con)/3.0
        scores.append({"cell_id": int(cid), "score": float(max(0.0, min(1.0, s)))})
    return {"studio": studio, "scores": scores}

# ---------- Simulations (toy) ----------
@app.get("/sim/run")
def sim_run(name: str = "Traffic Shift", delta: float = 0.1):
    REQS.labels(endpoint="/sim/run").inc()
    with db() as conn:
        rows = conn.execute("SELECT uc.id, fi.efficiency, fi.resilience, fi.connectivity FROM urban_cells uc LEFT JOIN fabric_indicators fi ON fi.cell_id=uc.id ORDER BY uc.id").fetchall()
    feats = []
    for (cid, eff, res, con) in rows:
        eff = float(eff or 0.6); res=float(res or 0.6); con=float(con or 0.6)
        if name.lower()=="traffic shift":
            con2 = max(0.0, min(1.0, con + delta*0.5))
            eff2 = max(0.0, min(1.0, eff + delta*0.2))
            res2 = res
        elif name.lower()=="transit uplift":
            con2 = max(0.0, min(1.0, con + delta*0.7)); eff2=eff; res2=res
        elif name.lower()=="heat island":
            res2 = max(0.0, min(1.0, res - delta*0.6)); eff2=eff; con2=con
        else:
            con2, eff2, res2 = con, eff, res
        feats.append({"cell_id": int(cid), "before": {"eff":eff,"res":res,"con":con}, "after": {"eff":eff2, "res":res2, "con":con2}})
    return {"name": name, "delta": delta, "results": feats}


# ## AUTOLOAD_TOOLS
# Auto-load tools from services/etl/tools if they expose register(app)
try:
    import importlib, pkgutil, os as _os
    from fastapi import APIRouter as _APIRouter
    _tools_pkg = "tools"
    _pkg_path = _os.path.join(_os.path.dirname(__file__), "tools")
    if _os.path.isdir(_pkg_path):
        for _finder, _name, _ispkg in pkgutil.iter_modules([_pkg_path]):
            try:
                _mod = importlib.import_module(f"{_tools_pkg}.{_name}")
                if hasattr(_mod, "register"):
                    _mod.register(app)
                    print("Loaded tool:", _name)
            except Exception as _ex:
                print("Tool load failed:", _name, _ex)
except Exception as _ex:
    print("Tools autoload error:", _ex)
